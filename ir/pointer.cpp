...
Pointer::Pointer(const Memory &m, const expr &bid, const expr &offset,
  ...
  : Pointer(m, bid, offset, bits_for_ptrattrs ? attr_to_bitvec(attr) : expr()){}
...
unsigned Pointer::totalBitsShort(bool short_bid) {
  return bits_for_bid - (short_bid && hasLocalBit()) + bitsShortOffset();
}
